...
namespace pk3DS.Core.Modding
{
    public static class XlsxResearchParser
    {
        ...
        {
            if (!File.Exists(xlsxPath)) return new List<string>();
        ...
        {
            if (!File.Exists(xlsxPath)) return new List<Dictionary<string, string>>();
            using var s = File.OpenRead(xlsxPath);
            return ReadSheet(s, sheetName);
        }

        public static List<Dictionary<string, string>> ReadSheet(Stream stream, string sheetName = "Sheet1")
        {
