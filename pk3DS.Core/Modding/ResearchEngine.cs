...
public static class ResearchEngine
{
    ...
    public static bool ExpandRelocationTable(string battlePath, string tableType, int expansionSize = 2000)
    {
        ...
        // 1. Locate Table (Ability Index or Move Index)
        // Signatures found in research
        byte[] sig = tableType == "Move" ? new byte[] { 0xD9, 0x02, 0x50, 0xE3 } : new byte[] { 0xE8, 0x00, 0x50, 0xE3 };
        ...
        int newTableOfs = oldLength;

        // 3. Move old table data and update count
        // (Simplified for now: filling with NOPs)
        byte[] empty = Enumerable.Repeat((byte)0x00, expansionSize).ToArray();
        empty.CopyTo(cro, newTableOfs);
        
        // Update the count instruction (CMP R0, #NewCount)
        int currentCount = tableType == "Move" ? 729 : 232;
        int newCount = currentCount + (expansionSize / 4);
        byte[] newCountBytes = BitConverter.GetBytes(0xE3500000 | (uint)newCount); // CMP R0, #new
        // Note: simplified, ARM literals have rotation logic. For 729+500, we might need a different instruction variant.
        
        File.WriteAllBytes(battlePath, cro);
        ProjectState.Instance.AppliedPatches.Add($"Relocate{tableType}Table");
