...
public class AesCounter
{
    ...
    {
        byte[] h = BitConverter.GetBytes(high);
        Array.Reverse(h);
        Array.Copy(h, Counter, 0x8);

        byte[] l = BitConverter.GetBytes(low);
        Array.Reverse(l);
        Array.Copy(l, 0, Counter, 0x8, 0x8);
    }

    public AesCounter(byte[] iv)
    {
        byte[] b = BitConverter.GetBytes(BitConverter.ToUInt64(iv, 0));
        Array.Reverse(b);
        Array.Copy(b, Counter, 0x8); 
    }
